...
banner:
  url: 'https://res.cloudinary.com/dccr9uiry/image/upload/dual-boot-bluetooth-sync-2_evpsqp.webp'
  pos: '0 40%'
---
# 前言
...
2. 掛載 Windows 的磁區
    ```sh
    sudo mount -t ntfs-3g /dev/your/windows/partition /mnt
    ```
3. 進入登錄檔的目錄
    ```sh
...
8. 列出當前目錄的內容時，應該會看到如下的資訊：
    ```txt title="" frame="terminal" showLineNumbers=false
